...
export default function MobileBottomNav() {
  ...
  return (
    <nav
      ...
      className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-rose-50/95 backdrop-blur-md border-t border-rose-150"
          ...
          return (
            <li key={n.href}>
              <Link
                href={n.href}
                className={
                  'flex flex-col items-center justify-center py-3 gap-0.5 transition relative tap-target focus-visible:outline-none focus-visible:bg-rose-100 ' +
              ...
              >
                <span className={'relative grid place-items-center ' + (active ? '' : 'opacity-80')}>
                  ...
                  {n.isCart && mounted && count > 0 && (
                    <span className="absolute -top-1.5 -right-2.5 bg-gold-500 text-ink-900 text-[11px] font-bold rounded-full w-4 h-4 grid place-items-center shadow">
                      {count}
                    </span>
                  )}
                </span>
                <span className="text-fluid-xs uppercase tracking-widest font-medium">{n.label}</span>
