...
export default function TrustPills({ variant = 'hero', className = '' }: TrustPillsProps) {
    ...
    return (
      <div className={`flex flex-wrap items-center gap-2 ${className}`}>
        {PILLS.map(({ icon: Icon, label }) => (
          <span
            key={label}
            className="inline-flex items-center gap-1.5 rounded-full bg-white border border-rose-150 px-3 py-1 text-[10px] uppercase tracking-widest text-ink-700"
          >
  ...
  if (variant === 'compact') {
    return (
      <div className={`grid grid-cols-1 sm:grid-cols-3 gap-3 p-4 rounded-2xl bg-rose-75 border border-rose-150 ${className}`}>
        {PILLS.map(({ icon: Icon, label }) => (
          <div key={label} className="flex items-center gap-2 text-xs text-ink-700">
  ...
  return (
    <div className={`flex flex-wrap items-center gap-x-5 gap-y-3 ${className}`}>
      {PILLS.map(({ icon: Icon, label }) => (
        <div key={label} className="inline-flex items-center gap-2 text-sm text-ink-700">
